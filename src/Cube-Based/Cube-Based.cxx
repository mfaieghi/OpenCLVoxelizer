...
//#define CROSS_SECTION_Y dim_grid[1]/2
...
#define KERNEL_FILE "..\\voxelization_kernel_over_cube.cl"
...
void print_device_info(cl_device_id dev) {
	...
	printf("CL_DEVICE_MAX_MEM_ALLOC_SIZE:         %lu Bytes\n", allocation_mem_size);
	printf("==========================================================\n");
...
int main()
{
	...
	long int voxel_counts = dim_grid[0] * dim_grid[1] * dim_grid[2];
	...
	/*Kernel preparation for the voxelization*/
	size_t voxelization_global_size[3] = { dim_grid[0], dim_grid[1], dim_grid[2] };
	//size_t local_size[3] = { max_local_size[0], max_local_size[1], max_local_size[2] };
	size_t voxelization_local_size[3] = { 8, 8, 8 };
	size_t voxelization_revised_global_size[3] = {};
	for (int i = 0; i < 3; i++){

		if (voxelization_global_size[i] % voxelization_local_size[i] != 0){
			voxelization_revised_global_size[i] = (voxelization_global_size[i] 
				/ voxelization_local_size[i] + 1) * voxelization_local_size[i];
		}
		else{
			voxelization_revised_global_size[i] = voxelization_global_size[i];
		}
	...
	cl_mem global_size_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY |
		CL_MEM_COPY_HOST_PTR, sizeof(size_t) * 3, voxelization_global_size, &err);
	if (err < 0) {
		printf("Error code: %d. Couldn't create the dimGrid_buffer\n", err);
		exit(1);
	};
	cl_mem dim_grid_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY |
		CL_MEM_COPY_HOST_PTR, sizeof(float) * 3, dim_grid, &err);
	if (err < 0) {
		printf("Error code: %d. Couldn't create the dimGrid_buffer\n", err);
		exit(1);
	};
	...
	int coords_size = triangles_count * 9;
	err = clSetKernelArg(voxelization_kernel, 0, sizeof(voxel_size), &voxel_size);
	err |= clSetKernelArg(voxelization_kernel, 1, sizeof(coords_size), &coords_size);
	err |= clSetKernelArg(voxelization_kernel, 2, sizeof(cl_mem), &dim_grid_buffer);
	err |= clSetKernelArg(voxelization_kernel, 3, sizeof(cl_mem), &min_corner_grid_buffer);
	...
	err = clEnqueueNDRangeKernel(queue, voxelization_kernel, 3, NULL, voxelization_revised_global_size,
	...
	void * density_mapped_memory = clEnqueueMapBuffer(queue, density_buffer, CL_TRUE,
		CL_MAP_READ, 0, sizeof(char) *  voxel_counts, 0, NULL, NULL, &err);
	if (err < 0) {
	...
	printf("Filled voxel count: %d", filled_voxel_count);
	...
	for (int voxel_index = 0; voxel_index < voxel_counts; voxel_index++) {
		if (density[voxel_index] >= 3 && density[voxel_index] <= 4) {
	...
	//voxelized_actor->GetProperty()->SetEdgeColor(1, 1, 1);
	printf("Filled voxel count: %d", filled_voxel_count);
	printf("Filled voxel count interior: %d", filled_voxel_count_interior);
#else
	vtkSmartPointer<vtkUnstructuredGrid> ug =
		vtkSmartPointer<vtkUnstructuredGrid>::New();
	vtkSmartPointer<vtkPoints> voxel_points =
