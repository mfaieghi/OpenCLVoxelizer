...
int main()
{
	...
	unsigned __int64 voxel_counts = dim_grid[0] * dim_grid[1] * dim_grid[2];
	unsigned __int64 array_length = voxel_counts / 32;
	if (array_length % 2 != 0) { array_length++; }
	...
	void * density_mapped_memory = clEnqueueMapBuffer(queue, density_buffer, CL_TRUE,
		CL_MAP_READ, 0, sizeof(unsigned int) * array_length, 0, NULL, NULL, &err);
	if (err < 0) {
		printf("Error code : %d. Couldn't map the buffer to host memory\n", err);
		exit(1);
	}
	memcpy(density, density_mapped_memory, sizeof(unsigned int) * array_length);
	...
	for (cl_uint array_index = 0; array_index < array_length; array_index++) {

		for (int n = 0; n < 32; n++) {
			int mask = 1 << n;
			int masked_density = density[array_index] & mask;
