...
#define STL_FILE_NAME "..\\model\\scapula.stl" 
...
//#define DOUBLE_ACTOR
//#define CROSS_SECTION_X 15*dim_grid[0]/20
...
void print_device_info(cl_device_id dev){
	...
	printf("CL_DEVICE_PROFILING_TIMER_RESOLUTION: %I64u ns\n", (int)time_res);
...
int main()
{
	...
	int voxel_counts = dim_grid[0] * dim_grid[1] * dim_grid[2];
	char *density = (char*)malloc(sizeof(char)*voxel_counts);
	for (int i = 0; i < voxel_counts; i++) {
		density[i] = 0;
	}
	...
	printf("Min and Max of Geometry: x_min = %f, y_min = %f, z_min = %f, \n x_max = %f, y_max = %f, z_max = %f\n",
		...
		max_corner_object[2]);
	printf("AABB: %.4f, %.4f, %.4f\n", max_corner_object[0] - min_corner_grid[0],
		max_corner_object[1] - min_corner_grid[1], 
		max_corner_object[2] - min_corner_grid[2]);
	printf("Voxel Size: %f, \n", VOXEL_SIZE);
	...
	cl_mem density_buffer = clCreateBuffer(context, CL_MEM_READ_WRITE |
		CL_MEM_COPY_HOST_PTR, sizeof(char)* voxel_counts, density, &err);
	if (err < 0) {
		printf("Error code: %d. Couldn't create the density_buffer\n", err);
		exit(1);
	};
	cl_kernel voxelization_kernel = clCreateKernel(program, "voxelizer", &err);
	if (err < 0) {
		printf("Error code: %d. Couldn't create voxelization_kernel\n", err);
		exit(1);
	};
	err = clSetKernelArg(voxelization_kernel, 0, sizeof(int), &voxelization_global_size);
	...
	cl_event map_event;
	void * density_mapped_memory = clEnqueueMapBuffer(queue, density_buffer, CL_TRUE,
		CL_MAP_READ, 0, sizeof(char) *  voxel_counts, 0, NULL, &map_event, &err);
	if (err < 0) {
		printf("Error code : %d. Couldn't map the buffer to host memory\n", err);
		exit(1);
	}
	memcpy(density, density_mapped_memory, sizeof(char) * voxel_counts);
	...
	size_t map_time_start, map_time_end, map_elapsed_time;
	clGetEventProfilingInfo(map_event, CL_PROFILING_COMMAND_START,
		sizeof(map_time_start), &map_time_start, NULL);
	clGetEventProfilingInfo(map_event, CL_PROFILING_COMMAND_END,
		sizeof(map_time_end), &map_time_end, NULL);
	map_elapsed_time = map_time_end - map_time_start;
	printf("Elapsed time in mapping: %I64u ns\n", (unsigned __int64)(map_elapsed_time));
	printf("Amount of data that is transferred: %I64u\n", (unsigned __int64)(voxel_counts));

	//cl_event write_event;
	//err = clEnqueueReadBuffer(queue, density_buffer, CL_TRUE, 0,
	//	sizeof(char) * voxel_counts, density, 0, NULL, &write_event);
	//if (err < 0) {
	//	printf("Error code : %d. Couldn't write into the density buffer\n", err);
	//	exit(1);
	//}

	//size_t write_time_start, write_time_end, write_elapsed_time;
	//clGetEventProfilingInfo(write_event, CL_PROFILING_COMMAND_START,
	//	sizeof(write_time_start), &write_time_start, NULL);
	//clGetEventProfilingInfo(write_event, CL_PROFILING_COMMAND_END,
	//	sizeof(write_time_end), &write_time_end, NULL);
	//write_elapsed_time = write_time_end - write_time_start;
	//printf("Elapsed time in mapping: %I64u ns\n", (unsigned __int64)(write_elapsed_time));
	//printf("Amount of data that is transferred: %I64u\n", (unsigned __int64)(voxel_counts * 4));
	...
	for (int voxel_index = 0; voxel_index < voxel_counts; voxel_index++) {
	...
	for (int voxel_index = 0; voxel_index < voxel_counts; voxel_index++) {
		if (density[voxel_index] == 5) {
			...
			int y = (voxel_index - z * dim_grid[0] * dim_grid[1]) / dim_grid[0];
			int x = voxel_index - z * dim_grid[0] * dim_grid[1] - y * dim_grid[0];
			if (x <= CROSS_SECTION_X && y <= CROSS_SECTION_Y && z <= CROSS_SECTION_Z) {
				float min_corner_voxel[3] = {
					min_corner_grid[0] + x * VOXEL_SIZE,
					min_corner_grid[1] + y * VOXEL_SIZE,
					min_corner_grid[2] + z * VOXEL_SIZE
...
#ifdef DOUBLE_ACTOR
	voxelized_actor_surface->GetProperty()->EdgeVisibilityOn();
	voxelized_actor_interior->GetProperty()->EdgeVisibilityOn();
