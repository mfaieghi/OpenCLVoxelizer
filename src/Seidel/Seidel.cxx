...
#define STL_FILE_NAME "D:\\GitHub\\OpenCLVoxelizer\\model\\Dragon.stl" 
...
#define KERNEL_FILE "D:\\GitHub\\OpenCLVoxelizer\\src\\Seidel\\Seidel.cl"
...
int main()
{
	...
	int dim_grid[3] = {
		ceil((max_corner_object[0] + 0.0001f - min_corner_object[0]) / VOXEL_SIZE),
		ceil((max_corner_object[1] + 0.0001f - min_corner_object[1]) / VOXEL_SIZE),
		ceil((max_corner_object[2] + 0.0001f - min_corner_object[2]) / VOXEL_SIZE)
	};
	...
	std::string fileName = "SoliVoxelization.txt";
	std::ofstream file;
	...
	for (int i = 0; i < dim_grid[0]; i++)
	{
		for (int j = 0; j < dim_grid[1]; j++)
		{
			for (int k = 0; k < dim_grid[2]; k++)
			{
				int index = i + j * dim_grid[0] + k * dim_grid[0] * dim_grid[1];
				if (density[index] >= 4)
				{
					voxel_center[0] = min_corner_object[0] + i * VOXEL_SIZE + HALF_VOXEL_SIZE;
					voxel_center[1] = min_corner_object[1] + j * VOXEL_SIZE + HALF_VOXEL_SIZE;
					voxel_center[2] = min_corner_object[2] + k * VOXEL_SIZE + HALF_VOXEL_SIZE;
					file << std::setw(10) << voxel_center[0] << std::setw(10) << voxel_center[1] << std::setw(10) << voxel_center[2] << std::setw(10) << 600 << std::endl;
				}
			}
		}
	}
