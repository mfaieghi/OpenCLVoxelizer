...
__kernel void voxelizer(int global_size,
	...
	__global unsigned int *density_1,
	__global unsigned int *density_2,
	__global unsigned int *density_3,
	__global unsigned int *density_4) {
			...
			for (int k = min_corner_ID.y; k <= max_corner_ID.y; k++) {
				for (int l = min_corner_ID.x; l <= max_corner_ID.x; l++) {
					...
					if (triBoxOverlap(voxel_size, center, v0, v1, v2)) {
						unsigned int voxel_index = l + k * dim_grid.x + j * dim_grid.x * dim_grid.y;
						__private unsigned int array_index = voxel_index / 32;
						if (array_index < 469762048) {
							__private unsigned int bit_pos = voxel_index % 32;
							__private unsigned int mask = 1 << bit_pos;
							atomic_or(&density_1[array_index], mask);
						}
						else if (array_index < 469762048 * 2){
							__private unsigned int bit_pos = voxel_index % 32;
							__private unsigned int mask = 1 << bit_pos;
							atomic_or(&density_2[array_index], mask);
						}
						else if (array_index < 469762048 * 3){
							__private unsigned int bit_pos = voxel_index % 32;
							__private unsigned int mask = 1 << bit_pos;
							atomic_or(&density_3[array_index], mask);
						}
						else{
							__private unsigned int bit_pos = voxel_index % 32;
							__private unsigned int mask = 1 << bit_pos;
							atomic_or(&density_4[array_index], mask);
						}
					}
				}
			}
